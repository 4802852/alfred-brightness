...
if brightness_levels_string:
    try:
        brightness_levels = [int(x) for x in brightness_levels_string.split(",") if int(x) >= 0]
    except ValueError:
        pass
value_min = 0
value_max = 100
...
def main(wf):
    ...
    error = 'Enter value between 0 and 100, or "%s" or "%s".' % (keyword_min, keyword_max)

    if user_input and not keyword_min.startswith(user_input) and not keyword_max.startswith(user_input):
        try:
    ...
    if show_min or (user_input and keyword_min.startswith(user_input)):
        wf.add_item("%s (%s%%)" % (keyword_min, value_min), arg="%s" % (value_min / 100.0), valid=True)

    if show_max or (user_input and keyword_max.startswith(user_input)):
        wf.add_item("%s (%s%%)" % (keyword_max, value_max), arg="%s" % (value_max / 100.0), valid=True)

    for i in brightness_levels:
        wf.add_item("%s%%" % i, arg="%s" % (i / 100.0), valid=True)

    try:
        cmd_out = os.popen("./brightness -l").read()
        cmd_out_line = cmd_out.split("\n")
        current_value = cmd_out_line[1].split("brightness ")[1]
